...
func TestOrganizationGroupsAISpend(t *testing.T) {
	...
	t.Run("MalformedGroupID", func(t *testing.T) {
		...
		// Given: a malformed UUID passed via raw HTTP (the typed client would reject it).
		// When: querying spend.
	...
	t.Run("GroupInOtherOrgExcluded", func(t *testing.T) {
		...
		// The second org is created via dbgen to avoid the multi-org premium check.
		db, ps := dbtestutil.NewDB(t)
		...
		resp, err := adminClient.OrganizationGroupsAISpend(ctx, group.OrganizationID, []uuid.UUID{group.ID, otherGroup.ID})
		require.NoError(t, err)
...
func TestOrganizationGroupsAISpendRoleAccess(t *testing.T) {
	...
	cases := []struct {
		Name                string
		Client              *codersdk.Client
		WantGroupInResponse bool
	}{
		{Name: "Owner", Client: ownerClient, WantGroupInResponse: true},
		{Name: "UserAdmin", Client: userAdminClient, WantGroupInResponse: true},
		{Name: "OrgAdmin", Client: orgAdminClient, WantGroupInResponse: true},
		{Name: "OrgUserAdmin", Client: orgUserAdminClient, WantGroupInResponse: true},
		// Plain org members are granted ResourceGroup.Read at the org scope
		// so they can share workspaces (see OrgMemberPermissions in
		// rbac/roles.go); dbauthz lets the row through.
		{Name: "Member", Client: memberClient, WantGroupInResponse: true},
	}
