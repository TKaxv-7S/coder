...
2. **Transport**: a non-PTY SSH exec channel obtained from the workspace `AgentConn` (`SSHClient`), preceded by a `command -v claude-agent-acp` preflight that turns a missing adapter into a legible config error. Because templates commonly install the adapter from a startup script and the turn dials the agent as soon as it connects, a failed probe is conclusive only once the agent's lifecycle state shows startup scripts settled (or the ready timeout elapses); until then the preflight re-probes on the workspace poll interval. Tests substitute an in-memory transport via `Config.ClaudeCodeTransport`.
