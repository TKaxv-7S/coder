...
func TestRegenerateChatTitle_PersistsAndBroadcasts(t *testing.T) {
	...
	db.EXPECT().GetChatSyntheticAPIKeyByUserID(gomock.Any(), ownerID).Return(database.ChatSyntheticApiKey{UserID: ownerID, APIKeyID: activeAPIKeyID}, nil)
	db.EXPECT().GetAPIKeyByID(gomock.Any(), activeAPIKeyID).Return(database.APIKey{ID: activeAPIKeyID, UserID: ownerID, ExpiresAt: time.Now().Add(48 * time.Hour)}, nil)
