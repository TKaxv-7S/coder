...
func (s *MethodTestSuite) TestAPIKey() {
	...
	s.Run("DeleteExternalAuthProviderConfig", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		id := uuid.New()
		dbm.EXPECT().DeleteExternalAuthProviderConfig(gomock.Any(), id).Return(nil).AnyTimes()
...
func (s *MethodTestSuite) TestUser() {
	...
	s.Run("GetExternalAuthProviderConfigByID", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		cfg := database.ExternalAuthProviderConfig{ID: uuid.New()}
		dbm.EXPECT().GetExternalAuthProviderConfigByID(gomock.Any(), cfg.ID).Return(cfg, nil).AnyTimes()
		check.Args(cfg.ID).Asserts(rbac.ResourceDeploymentConfig, policy.ActionRead).Returns(cfg)
	}))
	s.Run("GetExternalAuthProviderConfigByProviderID", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		cfg := database.ExternalAuthProviderConfig{ID: uuid.New(), ProviderID: "github"}
		dbm.EXPECT().GetExternalAuthProviderConfigByProviderID(gomock.Any(), cfg.ProviderID).Return(cfg, nil).AnyTimes()
		check.Args(cfg.ProviderID).Asserts(rbac.ResourceDeploymentConfig, policy.ActionRead).Returns(cfg)
	}))
	s.Run("GetExternalAuthProviderConfigs", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		cfgs := []database.ExternalAuthProviderConfig{{ID: uuid.New()}}
		dbm.EXPECT().GetExternalAuthProviderConfigs(gomock.Any()).Return(cfgs, nil).AnyTimes()
		check.Args().Asserts(rbac.ResourceDeploymentConfig, policy.ActionRead).Returns(cfgs)
	}))
	...
	s.Run("InsertExternalAuthProviderConfig", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		arg := database.InsertExternalAuthProviderConfigParams{ProviderID: "github", Type: "github"}
		cfg := database.ExternalAuthProviderConfig{ID: uuid.New(), ProviderID: arg.ProviderID, Type: arg.Type}
		dbm.EXPECT().InsertExternalAuthProviderConfig(gomock.Any(), arg).Return(cfg, nil).AnyTimes()
	...
	s.Run("UpdateExternalAuthProviderConfig", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		arg := database.UpdateExternalAuthProviderConfigParams{ID: uuid.New(), Type: "github"}
		cfg := database.ExternalAuthProviderConfig{ID: arg.ID, Type: arg.Type}
		dbm.EXPECT().UpdateExternalAuthProviderConfig(gomock.Any(), arg).Return(cfg, nil).AnyTimes()
		check.Args(arg).Asserts(rbac.ResourceDeploymentConfig, policy.ActionUpdate).Returns(cfg)
	}))
...
func (s *MethodTestSuite) TestSystemFunctions() {
	...
	s.Run("UpsertExternalAuthProviderConfigFromEnv", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		arg := database.UpsertExternalAuthProviderConfigFromEnvParams{ProviderID: "github", Type: "github"}
		cfg := database.ExternalAuthProviderConfig{ID: uuid.New(), ProviderID: arg.ProviderID, Type: arg.Type}
		dbm.EXPECT().UpsertExternalAuthProviderConfigFromEnv(gomock.Any(), arg).Return(cfg, nil).AnyTimes()
		check.Args(arg).Asserts(rbac.ResourceDeploymentConfig, policy.ActionUpdate).Returns(cfg)
	}))
